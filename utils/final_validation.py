...
def final_validation(config, model, data_loader, criterion, num_classes, class_names, neptune_logger=None):
    '''
    A function for the final validation of the model on the test data. Logs the following metrics to Neptune.ai:
        - Average test loss
        - Accuracy
        - F1 Score
        - Precision
        - Recall
        - AUC
    Makes the following visualizations and logs them to Neptune.ai:
        - Confusion Matrix
        - ROC Curves
    ...
    accuracy_metric = MulticlassAccuracy(num_classes=num_classes, average="macro").to(
        device
    )
    ...
    plot_confusion_matrix(
        config, all_targets, all_preds, class_names, neptune_logger=neptune_logger
    )

    plot_roc_curves(
        config, all_probs, all_targets, class_names, neptune_logger=neptune_logger
    )
...
def plot_roc_curves(config, all_probs, all_targets, class_names, neptune_logger):
    '''
    Plots, saves and logs the ROC curves of the micro and macro averages.
    
    ## Args:
        - config: configuration object
        - all_probs: model predictions
        - all_targets: target labels
        - class_names: list of class names
        - neptune_logger: Neptune logger object
    ...
    all_targets_binarized = label_binarize(
        all_targets.cpu().numpy(), classes=range(len(class_names))
    )
    ...
    fpr["micro"], tpr["micro"], _ = roc_curve(
        all_targets_binarized.ravel(), all_probs.ravel()
    )
...
def plot_confusion_matrix(config, all_targets, all_preds, class_names, neptune_logger):
    '''
    Plots, saves and logs the confusion matrix.
    ...
    fig, ax = plt.subplots(
        figsize=(max(len(class_names) / 5, 10), max(len(class_names) / 5, 10))
    )
    sns.heatmap(
        cm,
        annot=True,
    ...
    if neptune_logger is not None:
        neptune_logger.run["visualizations/confusion_matrix"].upload(
